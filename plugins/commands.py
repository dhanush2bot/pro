...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url="https://t.me/filmyspotmovie")
                ],[
    ...
    if len(message.command) != 2:
        buttons = [[
                    ...
                    InlineKeyboardButton('💳  𝐔𝐍𝐋𝐎𝐂𝐊 𝐏𝐑𝐄𝐌𝐈𝐔𝐌  💳', callback_data='dhanush') 
                ], [ 
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            f_caption=files1.caption
                    # Skip verification for PREMIUM_USER
        if not await check_verification(client, message.from_user.id) and VERIFY == True and not PREMIUM_USER:
            btn = [
                [
                ...
                [
                    InlineKeyboardButton('⁉️  ʜᴏᴡ ᴛᴏ ᴠᴇʀɪꜰʏ  ⁉️', url="https://t.me/filmyspotupdate/76")
                ]
                  ]
            await message.reply_text(
    ...
    if not files_:
        ...
        try:
            ...
            title = '@filmyspotmovie  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@filmyspotmovie  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if f_caption is None:
        f_caption = f"@filmyspotmovie  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
...
async def shortlink(bot, message):
    ...
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
       return await message.reply_text(f"<b>Hello {message.from_user.mention}!</b> 👋\n\n<i>Welcome to FilmySpot Movies! 🎬✨</i>\n\nReady to take your group experience to the next level? Here's how you can set up our exclusive link shortener:\n\n1️⃣ <b>Add me to your group as a Full Admin:</b> Just to make sure everything runs smoothly!\n\n2️⃣ <b>Send this magical command in your group:</b> <code>/shortlink moneykamalo.com 5ba307016aa86e8d8db02f3c4c63972f5062b0a2</code>\n\n<b>#Sample:</b> <code>/shortlink moneykamalo.com 5ba307016aa86e8d8db02f3c4c63972f5062b0a2</code>\n\nAnd voilà! Your link shortener is all set up. Enjoy the magic of FilmySpot!\n\nNeed help or have questions? Feel free to ask me anytime - @FilmySpotSupport_bot 🌟\n\n<i>(P.S. Don't miss out on our trusted earning site - [https://moneykamalo.com](https://moneykamalo.com))</i>", parse_mode='HTML')
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink moneykamalo.com 5ba307016aa86e8d8db02f3c4c63972f5062b0a2</code></b>")
